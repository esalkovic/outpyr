...
class TraceInspector:
    def __init__(self, trace_data_folder, data=None):
        ...
        self.df = csv_to_df(os.path.join(trace_data_folder, 'data_frame.csv'))
    ...
    def get_si_j_i(self, j, i): return nb_si(self.data[j, i], self.get_p_j(j), self.get_r_j(j))

    def get_log10si_j_i(self, j, i): return nb_log10si(self.data[j, i], self.get_p_j(j), self.get_r_j(j))
    ...
    def get_mu_j_trace(self, j):
        ...
        return get_mu(p_j_trace, r_j_trace)

    @skipped
    def get_var_j_trace(self, j):
        p_j_trace = self._get_param_j_trace('p', j)
        r_j_trace = self._get_param_j_trace('r', j)
        return get_var(p_j_trace, r_j_trace)
    ...
    def get_log10si_j_i_trace(self, j, i):
        ...
        for iteration, p in enumerate(p_j_trace):
            ...
            log10si_trace[iteration] = nb_log10si(x, p, r)
        return log10si_trace

    def plot_b_j_trace(self, j): plot(self.get_p_j_trace(j))

    def plot_e_j_trace(self, j): plot(self.get_p_j_trace(j))

    def plot_f_j_trace(self, j): plot(self.get_p_j_trace(j))

    def plot_p_j_trace(self, j): plot(self.get_p_j_trace(j))

    def plot_r_j_trace(self, j): plot(self.get_r_j_trace(j))

    def plot_mu_j_trace(self, j): plot(self.get_mu_j_trace(j))

    def plot_var_j_trace(self, j): plot(self.get_var_j_trace(j))

    def plot_si_j_i_trace(self, j, i): plot(self.get_si_j_i_trace(j, i))

    def plot_log10si_j_i_trace(self, j, i): plot(self.get_log10si_j_i_trace(j, i))
    ...
    def set_final_values_from_trace(self, genes=None):
        ...
        for _iteration, j in enumerate(genes):
            if j != 0:
                delete_print_lines(1)
            print('%s: Processing trace directory %s gene %d' % (now(), self.trace_data_folder, j))
            p_j_array = self.get_p_j_trace(j)
            p_j_array.shape = (p_j_array.size, 1)
            r_j_array = self.get_r_j_trace(j)
            r_j_array.shape = (r_j_array.size, 1)
            mu_j_array = get_mu(p_j_array, r_j_array)
            var_j_array = get_var(p_j_array, r_j_array)
            ...
            for i in range(self.data.shape[1]):
                self.v['p_values_mode'][j][i] = mode_of_continuous(cdf_[:, i], 10)
